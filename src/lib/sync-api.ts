...
export async function syncPrivacy(request: Request): Promise<Response> {
  try {
    ...
    const invalid = error instanceof z.ZodError || error instanceof SyntaxError || (error instanceof Error && error.message === "invalid_request");
    return authJson({ error: invalid ? "invalid_request" : "temporarily_unavailable" }, invalid ? 400 : 503);
  }
}
export async function privateSyncSummary(request: Request): Promise<Response> {
  ...
  try {
    ...
    if (result.error?.code === "22023") return authJson({ error: "invalid_request" }, 400);
