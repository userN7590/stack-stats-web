> Production v0.5.0: use the [audited deployment and E2E checklist](PRODUCTION_AUTH_SYNC.md).
> [Optional aggregate sync](SYNC.md) adds separately approved stats:write grants and refresh-token rotation. Identity-only linking remains unchanged; account connection alone never uploads history. Historical verification counts below refer to the original implementation.
