...
describe("private summaries and explicit publication", () => {
  it("treats invalid privacy bodies as client errors, without contacting the database", async () => {
    for (const [type, body] of [["text/plain", "{}"], ["application/json", "{"], ["application/json", JSON.stringify({ padding: "x".repeat(9000) })]]) {
      const response = await syncPrivacy(new Request("https://stackstats.dev/api/v1/sync/privacy", {
        method: "PUT", headers: { origin: "https://stackstats.dev", "content-type": type }, body,
      }));
  ...
  it("reports a database-rejected summary date as invalid rather than retryable", async () => {
    mocks.rpc.mockResolvedValueOnce({ data: null, error: { code: "22023", message: "private details" } });
    const response = await privateSyncSummary(new Request("https://stackstats.dev/api/v1/sync/summary?to=2099-01-01"));
    expect(response.status).toBe(400);
    expect(await response.json()).toEqual({ error: "invalid_request" });
  });
